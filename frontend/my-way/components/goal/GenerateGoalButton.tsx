...
type GenerateGoalButtonProps = {
    children?: React.ReactNode;
    roadmapId?: number;
    fetchRoadmap: () => void;
};

const GenerateGoalButton: React.FC<GenerateGoalButtonProps> = ({ roadmapId, children, fetchRoadmap }) => {
    ...
    return (
        <Button
            ...
            onPress={async () => {
                if (!roadmapId) {
                    alert(`Roadmap ID ${roadmapId} não fornecido.`);
                    return;
                }

                setIsLoading(true);
                await generateGoal(roadmapId);
                setIsLoading(false);
                fetchRoadmap();
            }}
            endContent={brainIcon({})}
        >
            Gere um novo objetivo com IA
        </Button>
    );
};

export default GenerateGoalButton;


const generateGoal = async (roadmapId: number) => {
    try {
        const response = await axios.post(
            `http://localhost:8081/ai/goal/gen-and-save/${roadmapId}`
        );

        if (response.status != 201) {
            alert('Erro ao gerar objetivo.');
        }
...
type GoalSvgIconProps = {
    width?: string;
    height?: string;
    color?: string;
};

const brainIcon: React.FC<GoalSvgIconProps> = ({
    width = "512.000000pt",
    height = "512.000000pt",
    color = "#000000",
}) => (
